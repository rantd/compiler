...
typedef enum {
	...
	expr_kind_app,		/* 関数呼び出し(f(1, 2)など. ただし 1 + 2 などもこの一種と見なす) */
} expr_kind_t;
...
/* コード生成に使われる任意の情報へのポインタ. */
struct syntree_info
{
    int save_location;  /* 保存先（メモリ or レジスタ） */
    int label;          /* ラベル生成 */
    int compound;       /* どのブロック(compound)に属しているか */
    int node_tree;      /* 木のノードに番号を割り当て、計算の格納場所を決める */
};
...
/* 関数定義のコンストラクタ */
fun_def_t mk_fun_def(char *filename, int line, char *f, var_decl_list_t params, stmt_t body);
