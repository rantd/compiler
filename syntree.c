...
static expr_t alloc_expr(char * filename, int line, expr_kind_t kind)
{
  ...
  e->line = line;
  e->kind = kind;
  e->info = (syntree_info_t) safe_malloc(sizeof(struct syntree_info));
  return e;
}
...
var_decl_t mk_var_decl(char * filename, int line, char * v)
{
  ...
  d->v = v;
  d->info = (syntree_info_t) safe_malloc(sizeof(struct syntree_info));
  return d;
}
...
static stmt_t alloc_stmt(char * filename, int line, stmt_kind_t kind)
{
  ...
  s->line = line;
  s->kind = kind;
  s->info = (syntree_info_t) safe_malloc(sizeof(struct syntree_info));
  return s;
}
...
stmt_t mk_stmt_compound(char * filename, int line, var_decl_list_t decls, stmt_list_t body)
{
